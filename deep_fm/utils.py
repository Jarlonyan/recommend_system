...
class SparseFeat(namedtuple('SparseFeat', ['name', 'dimension', 'use_hash', 'dtype', 'embedding_size', 'embedding'])):
    __solots__ = ()
...
class MultihotSparseFeat(namedtuple('MultihotSparseFeat', ['name', 'dimension', 'maxlen', 'combiner', 'use_hash', 'dtype', 'embedding_name', 'embedding'])):
    ...
    def __new__(cls, name, dimension, maxlen, combiner='mean', use_hash=False, dtype='float32', embedding_name=None, embedding=True):
        if embedding_name is None:
            embedding_name = name
        return super(MultihotSparseFeat, cls).__new__(cls, name, dimension, maxlen, combine, use_hash, dtypel, embedding_name, embedding)


def build_input_features(feature_columns, include_varlen=True, mask_zero=True, prefrix='', include_fixlen=True):
    ...
    if include_fixlen:
        for feat in feature_columns:
            ...
            elif isinstance(feat, MultihotSparseFeat):
